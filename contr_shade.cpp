...
This version has 4 switches, as follows:
showscale = 1 to show a color bar, 0 otherwise
lowcolor = 1 to color whole domain with lowest color, 0 otherwise
hatch = 1 to use diagonal hatching instead of colors
(normally this should be used with nl = 1 and lowcolor = 0)
plotcontour = 1 to outline contours, 0 otherwise.
Updated August 2011
*********************************************************************/
...
//Set up colors using Matlab 'jet' scheme - ******** need to replace with FMIN and FMAX ***********
	for(k=0; k<=nl; k++){
		xz = float(k)/float(nl);
